...
Say you have a variable `$items` storing 2 user profile objects:
...
This is how a column is added - exporter columns are added via the `$builder->add()` method. 
...
Let's back to the types. The library contains the following field types:
...
Here I only illustrate the use of the `MapType`. For details of other types please refer to the source code.
```
...
To put it in another way, field types are actually value transformers:
 ...
 * `StringType` provides an optional `format` configuration which will be passed to PHP's `sprintf()` when set.
...
The exporter is responsible for iterating the data set by the defined column set, and extract the record value of the column with the help of field type. Similarly, the exporter does not handle the record writing task itself. Instead, it delegates the task to an **output adapter**.
...
By default, columns are exported in the order when the fields are added. But you may want to modify the column order after the exporter is built:
```
$columnSet = $exporter->getColumns();
$columnSet->getChild('lastName')->setPosition(2);
```
Note that column position is not exclusive. It is legitimate to have nonconsecutive position, or two column with the same position. Alternately you can re-order the columns by using the `setColumnOrders()` method:
```
...
Or, you can even disable columns not specified in `setColumnOrders` by passing `true` as the second argument:   
```
